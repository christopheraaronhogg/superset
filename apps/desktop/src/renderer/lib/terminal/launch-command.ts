...
export async function writeCommandsInPane({
	...
	const command = buildTerminalCommand(commands, { shell, platform });
	if (!command) return;
...
/**
 * Prefer this over {@link launchCommandInPane} when the backend can chain
 * commands with the session's real shell (PowerShell short-circuit, cmd `&&`).
 */
